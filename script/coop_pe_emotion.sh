#!/bin/bash

echo "#######################################################################"
echo "##                  TRAINING EMOTION ON COOP 15                      ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_coop_15.json

echo "#######################################################################"
echo "##                  TRAINING EMOTION ON COOP 20                      ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_coop_20.json

echo "#######################################################################"
echo "##                  TRAINING EMOTION ON COOP 25                      ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_coop_25.json


echo "#######################################################################"
echo "##                  TRAINING EMOTION ON VPT 10                       ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_vpt_10_cn15.json

echo "#######################################################################"
echo "##                  TRAINING EMOTION ON VPT 10                       ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_vpt_10_cn20.json

echo "#######################################################################"
echo "##                  TRAINING EMOTION ON VPT 10                       ##"
echo "#######################################################################"
python3 coop_train.py config/coop/emotion/PE_vpt_10_cn25.json

echo "#######################################################################"
echo "##                        TRAINING FINISHED                          ##"
echo "#######################################################################"

echo "#######################################################################"
echo "##                        START TESTING                             ##"
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 0 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/coop/Emotion_cntx_15/ckpt/"
                    --batch_size 128
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 0 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/coop/Emotion_cntx_20/ckpt/"
                    --batch_size 128
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 0 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/coop/Emotion_cntx_25/ckpt/"
                    --batch_size 128
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 10 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/vpt/Emotion_cntx_15_vpt_10/"
                    --batch_size 128
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 10 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/vpt/Emotion_cntx_20_vpt_10/"
                    --batch_size 128
echo "#######################################################################"
python3 test_new.py --model_type "PECoreVPT" \
                    --num_prompt 10 \
                    --dataset_path "../processed_datasets/datasets_with_standard_labels/RAF-DB" \
                    --ckpt_dir "../TRAIN/PECore/L14/vpt/Emotion_cntx_25_vpt_10/"
                    --batch_size 128