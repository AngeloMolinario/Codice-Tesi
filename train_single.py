...
def get_loss_fn(config, weights=None):    
    ...
    if task == 0:
        return OrdinalPeakedCELoss(num_classes=len(config.CLASSES[0]), weights=weights, alpha=0.0, ce_weight=1.0)
    elif task == 1:
...
def single_task_val_fn(model, dataloader, loss_fn, device, task_weight, config, text_features=None):
    ...
    return [mean_loss], [accuracy], [all_preds], [all_labels], [all_probs]
...
def main():
    ...
    for epoch in range(config.EPOCHS):
        ...
        tracker.update_accuracy(None, train_acc[0], train=True, mean=True)
