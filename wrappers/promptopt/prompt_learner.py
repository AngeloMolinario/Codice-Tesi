...
class ClassPromptLearner(nn.Module):
    '''
        This class is used to learn the context vectors for the specific task.
        NOTE: This the the PromptLearner used in the original CoOp implementation.
    '''
...
class CoopModel(nn.Module):
    def __init__(self, n_ctx, classes, model, tokenizer):
        ...
        self.prompt_learner = ClassPromptLearner(n_ctx, classes, model.text_model, tokenizer, verbose=False)
        ...
        self.num_classes = [len(x) for x in classes] 

    def get_softCPT_parameters(self):
        """
        Returns the parameters of the prompt learner and the task prompt learner.
        """
        return list(self.prompt_learner.parameters())
    
    def forward(self, image):

        image_features = self.get_image_features(image, normalize=True)
        text_features = self.get_text_features(normalize=True)
    ...
    def get_text_features(self, normalize: bool = False):
        ...
        text_features = self.text_model.prompt_forward(prompt, tokenized_prompts)
    ...
    def save_logit(self, output_dir, filename="logits.pt"):
        ...
        save_path = os.path.join(output_dir, filename)
        torch.save(out_sd, save_path)
        print(f"[CoopCustomModel] logit_scale[/bias]) to {save_path}")
    ...
    def save_vision_model(self, output_dir: str, filename: str = "vision_ckpt.pt"):
        ...
        save_path = os.path.join(output_dir, filename)
        torch.save(out_sd, save_path)
        print(f"[CoopCustomModel] Vision model saved ({prefix}* + logit_scale[/bias]) to {save_path}")
